...
offlinewiki-daemon() {
	~/path/to/folder/for/kiwix/./kiwix-serve --port 49849 --daemon ~/path/to/data/zim/*.zim ;
}
...
zork() {
	frotz ~/path/to/Zork/DATA/ZORK1.DAT
}
...
opendisc() {
	sudo cryptsetup --type luks open /dev/sdX1 encryptedDisk
	sudo mount -t ext4 /dev/mapper/encryptedDisk /media/encryptedDisk
}
